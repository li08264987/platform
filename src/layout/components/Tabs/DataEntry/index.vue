<template>
  <div style="height:90%;">
    <div :class="{hasTagsView:needTagsView}" style="height:100%;margin-left:10px;">
      <dataSideBar v-if="showBar" class="sidebar-container" :bitems="dataRouter" />
...
export default {
  ...
  mounted() {
    ...
    routes.forEach(element => {
      if (element.path && (element.path === '/dataSystem')) {
        return
      }
    })
    ...
    monitorapi.getSideBar().then(res => {
      ...
      if (res.state === 1) {
        ...
        gn.forEach(element => {
          ...
          switch (code) {
            case 'ky' :
              this.dataRouter[0].children.push(
                {
                  path: 'kongya',
                  component: () => kongya,
                  name: 'DataKongya',
      ...
      console.log(err)
      self.dataRouter[0].children.push(
        {
          path: 'kongya',
          component: () => kongya,
          name: 'DataKongya',
